...
class NotificationBaseForAI(BaseModel):
    ...
    body: str = Field(description="ユーザーに表示される通知本文（手短で具体的、50-100文字程度、日本語で記述。すぐに実行できるアクションや問いかけを含める）")
    ai_context: str = Field(description="ユーザーに表示されない内部情報（通知生成の判断根拠、タスクの詳細分析、システム用のメタデータなど、日本語で記述）。due_dateの情報は含めないこと")
    due_date: datetime = Field(description="通知を送るタイミング（ISO形式: 2024-10-15T10:00:00）。")
