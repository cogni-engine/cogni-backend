...
prompt_template = ChatPromptTemplate.from_messages([
    (
        ...
        "\n\n【AIタスク（AI自動実行タスク）】"
        "\nis_ai_taskフィールドで、AIが実行するタスクか人間が実行するタスクかを判定してください："
        "\n- is_ai_task=true: AIが自動で調査・分析・実行するタスク"
        "\n  例: 「毎朝ニュースをまとめる」「市場動向を調査する」「データを分析する」"
        "\n- is_ai_task=false: 人間が実行するタスク（デフォルト）"
        "\n  例: 「資料を作成する」「会議に出席する」「メールを送る」"
        "\n※ 判断基準: AIが自律的に実行可能で、人間の判断や物理的な行動が不要ならtrue"
...
- 各タスクに以下の情報を含めてください:
  ...
  - is_ai_task: AIが自動実行するタスクの場合はtrue、人間が実行するタスクの場合はfalse
